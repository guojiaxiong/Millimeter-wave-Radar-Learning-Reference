...
for i = 1 : numframe
    temp = dca();%96x4x192->96x8x96
                                                                                                                                                                 
    for j = 1:ntx                                             
        TempData(:,:,:,j)  = temp(:,:,j:ntx:end); %96x4x96x2
        TempData2=permute(TempData,[1 3 2 4]);%96*4*2*96

    end
    Temp=reshape(TempData2,96,96,[]);
    Temp_reshaped(i,:,:,:)=Temp;
    % iqData = [iqData,temp];%96x8x32->96x8x32x1200  
end
% iqData=squeeze(Temp_reshaped(:,:,1,:));
iqData=Temp_reshaped;
filename=fullfile('E:\UESTC\kb406\TI-数据集\呼吸检测\采集数据（60s)','iqData3.mat');
