function current_points = tracks_ultra(detections, p)
    % 输入参数:
    ...
    proximityThreshold = 1;             % 邻近点阈值，未匹配成功的点和匹配成功的点距离小于阈值，不建新轨迹
    clusterThreshold = 1.5;             % 第一帧点的距离阈值，小于视为一个目标建一条轨迹，大于视为两个目标，建两条轨迹
    costThreshold = p.costThreshold;
    predictionframe = p.predictionframe;
    reset = p.reset;

    % ===== 新增参数 =====
    staticSpeedThreshold = 0.05; % 静止状态的速度阈值
    staticPosThreshold = 0.1;    % 静止状态的位置变化阈值
    staticFramesThreshold = 3;   % 连续静止帧数阈值
    % ===================
    ...
    % 初始化检查
    % tracks结构体（'id', 轨迹id, 'x', 轨迹最新点位置速度信息, 'P', {}, 'age',轨迹年龄,从第一次出现开始计数，每帧加1 ...
    %                     'totalVisibleCount', 轨迹总的匹配上的点数, 'consecutiveInvisibleCount', 连续没有匹配上新点的帧数, ...
    %                     'history',轨迹历史位置信息, 'dirChangeRate', {}, ...
    %                     'staticCount', 连续静止帧数, 'isStatic', 运动状态1静止2行走3跑步）
    if isempty(tracks) || reset
        tracks = struct('id', {}, 'x', {}, 'P', {}, 'age', {}, ...
                    'totalVisibleCount', {}, 'consecutiveInvisibleCount', {}, ...
                    'history', {}, 'dirChangeRate', {}, ...
                    'staticCount', {}, 'isStatic', {}); % 添加静止状态字段
    ...
    if numTrks > 0
        ...
        activeTrks = true(1, numTrks); % 标记活动轨迹 1是活动轨迹，0是休眠状态轨迹（连续三帧没有新点匹配上的轨迹）
        ...
        for i = 1:numTrks
            ...
            % 添加当前状态到历史
            stateHistories{i} = [stateHistories{i}, tracks(i).x];%轨迹的历史状态
            ...
            if size(stateHistories{i}, 2) > predictionframe
                stateHistories{i} = stateHistories{i}(:, end-predictionframe+1:end);%轨迹历史状态，限制轨迹的长度
            end
    ...
    if all(isinf(cost(:)))
        ...
        [assignments, unTrks, unDets] = matchpairs(cost, costThreshold, 'min'); %assignments=[轨迹id 当前检测点id]
    end
    %删除隔得远的匹配结果
    id = [];
    for i = 1:size(assignments, 1)
        if ~isempty(assignments)
            ...
            trackedPos = tracks(trkIdx).x(1:2); 
            detectedPos = currentDets(detIdx, 1:2)'; 
            distance = norm(trackedPos - detectedPos);
            if distance > 1.5 
                id = [id; i];
            end
        end
    ...
    for i = 1:length(tracks)
        % 检查速度是否低于阈值
        speed = norm(tracks(i).x(3:4),'fro');
        if speed < staticSpeedThreshold
            % 检查位置变化是否低于阈值（使用历史位置）
            if size(tracks(i).history, 1) >= 2
                % 计算最近两帧的位置变化
                posChange = norm(tracks(i).history(end, :) - tracks(i).history(end-1, :));
                if posChange < staticPosThreshold
                    tracks(i).staticCount = tracks(i).staticCount + 1;
                else
                    tracks(i).staticCount = 0;
                end
            else
                tracks(i).staticCount = tracks(i).staticCount + 1;
            end
        ...
        % 更新静止状态标志
        if tracks(i).staticCount >= staticFramesThreshold
            tracks(i).isStatic = 1;%超过三帧都判断为静止才变为静止
        elseif speed < staticSpeedThreshold + 1
            tracks(i).isStatic = 2;
        else
            tracks(i).isStatic = 3;
        end
    ...
    for i = 1:length(tracks)
        ...
        % 保持历史长度（最多保留staticFramesThreshold+5帧）
        if size(tracks(i).history, 1) > staticFramesThreshold + 5
            tracks(i).history = tracks(i).history(end-staticFramesThreshold-4:end, :);
        end
    end
    ...
    current_points = [];
    for i = 1:length(tracks)
        current_points(i,:)  = [tracks(i).id tracks(i).x(1:2)' tracks(i).isStatic];
    end
end