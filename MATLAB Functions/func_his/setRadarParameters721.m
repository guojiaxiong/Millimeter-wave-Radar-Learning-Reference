function parameter = setRadarParameters721()
% 设置基础雷达参数
parameter.frameWindow = 1;
parameter.c = 3e8;                  % 光速
parameter.stratFreq = 24.025e9;     % 起始频率
parameter.Tr = 103e-6;              % chirp上升时间
parameter.Tc = 400e-6;              % chirp间隔时间
parameter.Fs = 2.5e6;               % 采样率
parameter.Dsample = 1;              % 降采样倍数
parameter.Chirps = 128;              % chirp数
parameter.frameTime = 100e-3;      % 帧时间
parameter.Bandwidth = 250e6;        % 发射信号有效带宽
parameter.Samples = 256;            % 采样点
parameter.fftSamples = 32;            % 1dfft采样点
...
parameter.velocitycut = 2.5;             % 速度保留位
...
parameter.lossR = 75;                % 滤除杂波
parameter.lossD = 0.26;                % 滤除杂波
% 检测部分
...
% 帧关联参数
parameter.epsilon1 = 0.4;               % 邻域半径
parameter.minPoints1 = 1;               % 形成簇的最小点数
...
%轨迹运动状态映射
parameter.motionStateMap = containers.Map('KeyType', 'double', 'ValueType', 'any');
parameter.maxdets = 50;
parameter.mindets = 10;

% 导入测试场景
parameter.PersonNum = 3;
parameter.testscene = 3; 
if parameter.testscene == 0         % default = 0;
    parameter.x = [-5.00 5.00];
    parameter.y = [-0.00 8.00];
elseif parameter.testscene == 1     % Rooftop = 1;
    parameter.x = [-3 3];
    parameter.y = [0 8];
elseif parameter.testscene == 2     % Corridor = 2;
    parameter.x = [-2.40 2.40];
    parameter.y = [-1.00 9.00];
