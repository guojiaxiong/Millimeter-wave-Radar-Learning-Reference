function currenttracks = tracks_1f_plus(detections, params,costThreshold,predictionWindow, reset)
    % 输入参数:
    ...
    minInitialHits = params.minInitialHits;
    maxInvisibleCount = params.maxInvisibleCount;
    ...
    if numTrks > 0
        ...
        for i = 1:numTrks
            ...
            % 保持指定长度的历史
            if size(stateHistories{i}, 2) > predictionWindow
                stateHistories{i} = stateHistories{i}(:, end-predictionWindow+1:end);
            end
        end
    ...
    for a = 1:size(assignments, 1)
        ...
        if size(stateHistories{trkIdx}, 2) > predictionWindow
            stateHistories{trkIdx} = stateHistories{trkIdx}(:, end-predictionWindow+1:end);
        end
    end
