...
for i=1:NR
    modes_white_noise{i}=emd_auto(white_noise{i});%calculates the modes of white gaussian noise
end;

for i=1:NR %calculates the first mode
    xi=x+Nstd*modes_white_noise{i}(1,:)/std(modes_white_noise{i}(1,:));
    [temp, o, it]=emd_auto(xi,'MAXMODES',1,'MAXITERATIONS',MaxIter);
    temp=temp(1,:);
    aux=aux+(xi-temp)/NR;
    iter(i,1)=it;
end;
...
k=1;
aux=zeros(size(x));
es_imf = min(size(emd_auto(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter)));
...
while es_imf>1 %calculates the rest of the modes
    for i=1:NR
        ...
        if tamanio(1)>=k+1
            ...
            noise=Nstd*noise;
            try
                [temp,o,it]=emd_auto(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            catch    
                it=0; disp('catch 1 '); disp(num2str(k))
                temp=emd_auto(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            end;
            temp=temp(end,:);
        else
            try
                [temp, o, it]=emd_auto(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
            catch
                temp=emd_auto(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
